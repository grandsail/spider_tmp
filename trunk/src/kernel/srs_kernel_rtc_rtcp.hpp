...
class SrsRtcpACK : public SrsRtcpCommon
{
private:
    uint64_t time1_; //L1发送的rtp包的时间戳
    uint64_t time2_; //L2收到rtp包的时间
public:
    SrsRtcpACK();
    virtual ~SrsRtcpACK();
    ...
    uint64_t get_time1() const;
    uint64_t get_time2() const;
    void set_time1(uint64_t time);
    void set_time2(uint64_t time);
public:
    virtual srs_error_t decode(SrsBuffer *buffer);
    virtual uint64_t nb_bytes();
    virtual srs_error_t encode(SrsBuffer *buffer);
