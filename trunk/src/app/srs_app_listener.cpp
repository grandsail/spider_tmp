...
// TOFIXG-MONITOR: 此处在sendonly_skt上的修改暂时无用
void SrsUdpMuxSocket::set_addr(std::string ip, int port) {
    ...
    fromlen = sizeof(tmp);
    srs_trace("[BUPT DEBUG] set addr done. %s:%d", inet_ntoa(((struct sockaddr_in*)&from)->sin_addr), ntohs(((struct sockaddr_in*)&from)->sin_port));
}

void SrsUdpMuxSocket::set_session(SrsRtcConnection* session) {
    if (session == NULL) {
        srs_trace("<BTRACE> set session is null");
    } else {
        if (session->connection_type_ == 1) {
            srs_trace("<BTRACE> session type SstUpstreamPlayerSession");
        } else if (session->connection_type_ == 2) {
            srs_trace("<BTRACE> session type SstDownstreamPublisherSession");
        }
    }
    session_ = session;
}
