...
{
    is_sst_upstream_consumer = false;
    snum_ = -1;
    sdegree_ = 0;
    sst_id_ = -1;
    base_seq_ = 0;
...
srs_error_t SrsRtcConsumer::set_sst_header(SrsRtpPacket* pkt)
{
    srs_error_t err = srs_success;
    if (pkt == NULL)
    {
        return srs_error_new(ERROR_SST, "packet is null");
    }
    uint16_t osn = pkt->header.get_sequence();
    if (!base_is_set_)
    {
        srs_trace("<BTRACE> sst base is set to %u", osn);
        base_seq_ = osn;
        base_is_set_ = true;
    }
    pkt->header.set_sst_info(sst_id_, osn, snum_, sdegree_);
    pkt->header.set_sst_new_sn(base_seq_, sdegree_);
    return err;
}

srs_error_t SrsRtcConsumer::sub_stream_set_seq(SrsRtpPacket* pkt)
{
    srs_error_t err = srs_success;
    if (pkt == NULL)
    {
    ...
    if (!base_is_set_)
    {
        srs_trace("<BTRACE> sub stream seq base is set to %u", osn);
        base_seq_ = osn;
        base_is_set_ = true;
    }
    pkt->header.set_sequence(base_seq_);
    base_seq_ += 1;
    return err;
}

bool SrsRtcConsumer::sub_stream_should_forward(int bias, uint16_t seq)
{
    return send_map_[seq % SEND_MAP_LEN];
}

// srs_error_t SrsRtcConsumer::set_reinjection_rules(int start_reinject, int reinject_index)
// {
//     srs_error_t err = srs_success;
//     srs_trace("<BTRACE> set reinject rule, start/stop=%d, reinject index=%d", start_reinject, reinject_index);
//     if (reinject_index < 0 || reinject_index >= sdegree_)
//     {
//         srs_warn("wrong reinject index");
//         return err;
//     }
//     if (start_reinject > 0)
//     {
//         redundant_[reinject_index] = true;
//     }
//     else if (start_reinject < 0)
//     {
//         redundant_[reinject_index] = false;
//     }
//     return err;
// }

srs_error_t SrsRtcConsumer::init_send_map(int snum, int sdegree)
{
    srs_error_t err = srs_success;
    srs_trace("<BTRACE> init send map with snum=%d, sdegree=%d", snum, sdegree);
    for (int i = 0; i < SEND_MAP_LEN; i ++)
    {
        if (i % sdegree == snum)
        {
            send_map_[i] = true;
...
srs_error_t SrsRtcConsumer::set_send_map(string send_map)
{
    srs_error_t err = srs_success;

    if (send_map.size() != SEND_MAP_LEN) {
        return srs_error_new(ERROR_SST, "send map length wrong");
    }
...
{
    // printf("<DEBUG> enqueue seq=%u\n", pkt->header.get_sequence());
    srs_error_t err = srs_success;
    ...
    // {
    //     highest_seq_ = seq;
    //     srs_trace("<BTRACE> set highest seq to %u", seq);
    //     highest_is_set_ = true;
    // }
    // ring_buffer[seq] = pkt;
    // ring_payload_ += 1;
    // if (mw_waiting)
    // {
    //     if (ring_payload_ > mw_min_msgs)
    //     {
    //         srs_cond_signal(mw_wait);
    //         mw_waiting = false;
    //         return err;
...
srs_error_t SrsRtcConsumer::dump_packet(SrsRtpPacket** ppkt)
{
    ...
    //     return err;
    // }
    // while (mw_min_msgs != 0 && ring_payload_ > mw_min_msgs && ring_buffer[highest_seq_] == NULL)
    // {
    //     // printf("<DEBUG> redundant, continue highest_seq_=%u\n", highest_seq_);
    //     highest_seq_ += 1;
    // }
    // if (ring_buffer[highest_seq_] != NULL)
    // {
    //     SrsRtpPacket* pkt = ring_buffer[highest_seq_];
    ...
    //         pre_ts_ = ts;
    //     }
    //     ring_buffer[highest_seq_] = NULL;
    //     // printf("<DEBUG> dump packet seq=%u, payload=%d, ts=%u\n", highest_seq_, ring_payload_, ts);
    //     ring_payload_ -= 1;
    //     highest_seq_ += 1;
    //     return err;
    // }
    // printf("<DEBUG> ring_payload_=%d, mw_min_msgs=%d, highest_seq_=%u, highest_null=%d\n", ring_payload_, mw_min_msgs, highest_seq_, ring_buffer[highest_seq_] == NULL);
    // ****** BUPT ******
...
{
    // printf("wait %d packets, payload=%d\n", nb_msgs, ring_payload_);
    mw_min_msgs = nb_msgs;
...
{
    sst_base_is_set = false;
    sst_base_sn = 0;
    called_from_sst_player_ = false;

    is_sst_downstream_source_ = false;
    get_desc_from_publisher = false;
    sst_lead_audio_ssrc_ = 0;
    sst_lead_video_ssrc_ = 0;

    convert_seq_ = 0;
    cdn_fec_pkt_counter_ = 0;
    header_conversion_is_inited_ = false;
    convert_gno_ = 0;
    ...
    pre_arrive_seq_ = MAX_SEQ_LIMIT;
    memset(is_arrived_, 0, sizeof(is_arrived_));
}
...
SrsRtcSource::~SrsRtcSource()
{
    ...
    srs_freep(fec_decoder_);
    srs_freep(fec_encoder_);
}
...
{
    srs_trace("<BTRACE> on source changed");
    srs_error_t err = srs_success;

    // Update context id if changed.
    // TOFIXG-MERGE: 研究source_id的逻辑
    ...
    for (it = consumers.begin(); it != consumers.end(); ++it) {
        ...
        srs_trace("<BTRACE> SrsRtcSource::on_source_changed() notify consumer");
        trace_sst_descs();
...
bool SrsRtcSource::lead_is_set() {
    return sst_lead_audio_ssrc_ != 0 && sst_lead_video_ssrc_ != 0;
}

srs_error_t SrsRtcSource::sst_on_source_changed() {
    srs_error_t err = srs_success;
    if (consumers.size() <= 0) {
        return srs_error_new(ERROR_SST, "SrsRtcSource::sst_on_source_changed() with no consumer");
    }
    called_from_sst_player_ = true;
    err = on_source_changed();
    called_from_sst_player_ = false;
    return err;
}

SrsRtcTrackDescription* SrsRtcSource::get_desc_witch_ssrc(uint32_t ssrc)
{
    SrsRtcTrackDescription* track = stream_desc_->find_track_description_by_ssrc(ssrc);
    if (track) {
        // 主track命中
        return track;
    }
    track = get_additional_desc_with_ssrc(ssrc);
    return track;
}

SrsRtcTrackDescription* SrsRtcSource::get_additional_desc_with_ssrc(uint32_t ssrc)
{
    for (std::vector<SrsRtcSourceDescription*>::iterator it = sst_additional_descs_.begin(); it != sst_additional_descs_.end(); it ++) {
        SrsRtcTrackDescription* track = (*it)->find_track_description_by_ssrc(ssrc);
        if (track) {
            return track;
        }
    }
...
void SrsRtcSource::trace_sst_descs()
{
    srs_trace("<BTRACE> SrsRtcSource::trace_sst_descs() main audio %u, main video %u", stream_desc_->audio_track_desc_->ssrc_, stream_desc_->video_track_descs_[0]->ssrc_);

    for(std::vector<SrsRtcSourceDescription*>::iterator it = sst_additional_descs_.begin(); it != sst_additional_descs_.end(); it ++) {
        srs_trace("<BTRACE> SrsRtcSource::trace_sst_descs() additional audio %u, additional video %u", (*it)->audio_track_desc_->ssrc_, (*it)->video_track_descs_[0]->ssrc_);
    }
}

srs_error_t SrsRtcSource::cdn_fec_header_conversion(SrsRtpPacket *pkt)
{
    // 更改序列号和ssrc
    srs_error_t err = srs_success;
    if (!header_conversion_is_inited_)
    {
        convert_seq_ = pkt->header.get_sequence();
        // NOTEG: 只能处理纯视频流
        cdn_fec_ssrc_ = pkt->header.get_ssrc();
        header_conversion_is_inited_ = true;
        srs_trace("<BTRACE> source convert seq init=%u, convert ssrc=%u", convert_seq_, cdn_fec_ssrc_);
    }
    pkt->header.set_sequence(convert_seq_);
    convert_seq_ += 1;
    if (pkt->header.get_ssrc() == 0)
    {
        pkt->header.set_ssrc(cdn_fec_ssrc_);
    }
    return err;
}

bool SrsRtcSource::have_arrived(uint16_t seq)
{
    // 此处应为真实的seq，在下游时即为gno
    if (MAX_SEQ_LIMIT <= pre_arrive_seq_ || pre_arrive_seq_ > seq + MAX_SEQ_LIMIT / 2) {
        srs_trace("<BTRACE> reset arrived map: seq=%u, pre_arrive_seq=%u", seq, pre_arrive_seq_);
        memset(is_arrived_, false, sizeof(is_arrived_));
    }
    pre_arrive_seq_ = seq;
    return is_arrived_[seq];
}
...
{
    if (!is_sst_downstream_source_ || !publish_stream_) {
        publish_stream_ = v;
        fec_encoder_ = new SrsFecEncoder(DEFAULT_FEC_BUFFER_SIZE, DEFAULT_FEC_METHOD);
        fec_decoder_ = new SrsFecDecoder(DEFAULT_FEC_BUFFER_SIZE, DEFAULT_FEC_METHOD);
    }
    else {
        sst_additional_publishers_.push_back(v);
    }
}
...
srs_error_t SrsRtcSource::on_rtp(SrsRtpPacket* pkt)
{
    ...
    // save original information for NACK check
    uint8_t opno;
    uint16_t ogno;
    uint32_t ossrc = pkt->header.get_ssrc();
    uint16_t oseq = pkt->header.get_sequence();
    if (is_sst_downstream_source_) { 
        opno = pkt->header.get_pf_pno();
        ogno = pkt->header.get_pf_gno();
        // srs_trace("<DEBUG> get packet: seq=%u, pno=%d, gno=%u", pkt->header.get_sequence(), opno, ogno);
    }

    // get info in sst headers
    if (is_sst_downstream_source_) {
        // only peers will get in here
        // if ((err = pkt->header.move_sst_osn_to_sn()) != srs_success) {
        //     return srs_error_wrap(err, "error during drop sst header");
        // }
        uint32_t ossrc = pkt->header.get_ssrc();
        // pkt->header.drop_sst();
        if (ossrc != sst_lead_audio_ssrc_ && ossrc != sst_lead_video_ssrc_) {
            SrsRtcTrackDescription* track = get_additional_desc_with_ssrc(ossrc);
            if (track) {
                if (track->type_ == "audio") {
                    pkt->header.set_ssrc(sst_lead_audio_ssrc_);
                } else if (track->type_ == "video") {
                    pkt->header.set_ssrc(sst_lead_video_ssrc_);
                }
                else {
                    return srs_error_new(ERROR_SST, "error when changing ssrc in SrsRtcSource::on_rtp(SrsRtpPacket* pkt)");
                }
            }
            else {
                srs_trace("<BTRACE> desc mismatch in source on rtp");
                trace_sst_descs();
                return srs_error_new(ERROR_SST, "descs in source mismatch: ossrc %u", ossrc);
            }
        }
    }

    // 在此处设置一二级之间的发送是否有空包
    bool EMPTY_PACKET_ENABLED = true;
    if (!is_sst_downstream_source_) {
        /***    CDN    ***/

        // NOTEG: 返回一个指针，检查是否为空
        SrsRtpPacket *redundant_pkt = fec_encoder_->put_and_encode(pkt->copy());
        cdn_fec_header_conversion(pkt);
        for (SrsRtcConsumer *consumer : consumers)
        {
            if (consumer->is_sst_upstream_consumer && (pkt->header.get_sequence() % consumer->sdegree_ == consumer->snum_))
            {
                SrsRtpPacket *pkt_to_send = pkt->copy();
                // consumer->set_sst_header(pkt_to_send);
                // TOFIXG: 目前征用了gno作为解码时的原始序列号
                pkt_to_send->header.set_pf_info(PF_ID, cdn_fec_pkt_counter_, convert_gno_);
                // srs_trace("<DEBUG> send packet: seq=%u, pno=%d, gno=%u", pkt_to_send->header.get_sequence(), pkt_to_send->header.get_pf_pno(), pkt_to_send->header.get_pf_gno());

                if ((err = consumer->enqueue(pkt_to_send)) != srs_success)
                {
                    return srs_error_wrap(err, "consumer message");
                }
            }
        }
        cdn_fec_pkt_counter_ += 1;
        convert_gno_ += 1;

        // 在此处更改是否使用冗余编码
        const bool REDUNDANT_ENABLED = true;
        if (redundant_pkt != nullptr && REDUNDANT_ENABLED)
        {
            cdn_fec_header_conversion(redundant_pkt);
            for (SrsRtcConsumer *consumer : consumers)
            {
                if (consumer->is_sst_upstream_consumer && (pkt->header.get_sequence() % consumer->sdegree_ == consumer->snum_))
                {
                    // consumer->set_sst_header(redundant_pkt);
                    // NOTEG: 此处convert_gno_不做自增, 冗余包 gno-pno=本FEC组第一个包的gno
                    redundant_pkt->header.set_pf_info(PF_ID, cdn_fec_pkt_counter_, convert_gno_);
                    // srs_trace("<DEBUG> send packet: seq=%u, pno=%d, gno=%u", redundant_pkt->header.get_sequence(), redundant_pkt->header.get_pf_pno(), redundant_pkt->header.get_pf_gno());

                    if ((err = consumer->enqueue(redundant_pkt->copy())) != srs_success)
                    {
                        return srs_error_wrap(err, "consumer message");
                    }
                }
        ...
        else if (redundant_pkt != nullptr) {
            cdn_fec_pkt_counter_ = 0;
        }
    } else {
        /***    PEER    ***/
        for (SrsRtcConsumer* consumer : consumers) {
            if (consumer->is_sst_upstream_consumer) 
            /***    CONSUMER TOWARDS DOWNSTREAMS    ***/
            {
                int8_t pno = pkt->header.get_pf_pno();
                uint16_t gno = pkt->header.get_pf_gno();
                if (!consumer->sub_stream_should_forward(pno, pkt->header.get_sequence()))
                {
                    continue;
                }
                SrsRtpPacket* pkt_to_send = pkt->copy();
                // consumer->set_sst_header(pkt_to_send);
                if (EMPTY_PACKET_ENABLED) {
                    // 开启一二级之间传输空包，标记子流
                    // osn为1则命中模除关系
                    uint16_t osn_to_set = 0;
                    if (pkt->header.get_sequence() % consumer->sdegree_ == consumer->snum_) {
                        osn_to_set = 1;
                    }
                    pkt_to_send->header.set_sst_info(consumer->sst_id_, osn_to_set, consumer->snum_, consumer->sdegree_);
                }
                pkt_to_send->header.set_pf_info(PF_ID, pno, gno);
                consumer->sub_stream_set_seq(pkt_to_send);

                struct timeval tv;    
                gettimeofday(&tv, NULL);
                uint64_t time = tv.tv_sec * 1000000 + tv.tv_usec ; 
                pkt_to_send->header.set_sendtime_info(SendTime_ID, time); //L1添加sendtime扩展头
                uint64_t read_time;
                pkt_to_send->header.get_sendtime_info(read_time);
                // srs_trace("<L1 TIME>pno:%d,time:%u",pno,read_time);
            ...
            /***    CONSUMER TOWARDS LOCAL BROWSER    ***/ 
            {
                // TOFIXG: 目前征用了gno作为解码时的原始序列号
                // TOFIXG: 目前只能1号包丢失时保证不掉帧
                int8_t pno = pkt->header.get_pf_pno();
                uint16_t gno = pkt->header.get_pf_gno();
                pkt->header.set_sequence(gno);

                // 只有二级处理空包
                if (EMPTY_PACKET_ENABLED && sst_additional_descs_.size() > 1) {
                    // 开启一二级之间传输空包
                    uint16_t osn;
                    uint8_t snum, sdegree;
                    pkt->header.get_sst_info(osn, snum, sdegree);
                    pkt->header.drop_sst();
                    if (osn == 0) {
                        // 忽略空包
                        // srs_trace("<DEBUG> ignore empty packet, snum=%u, pno=%u", snum, pno);
                        continue;
                    }
                    // srs_trace("<DEBUG> get usable packet, snum=%u, pno=%u", snum, pno);
                }

                if(have_arrived(gno))
                {
                    // srs_trace("<DEBUG> ignore seq=%u, pno=%d, gno=%u", pkt->header.get_sequence(), pno, gno);
                    continue;
                }

                fec_decoder_->put_decode_buffer(pkt->copy());

                // //L2读sendtime扩展头
                // uint64_t time;
                // pkt->header.get_sendtime_info(time); 
                // srs_trace("<L2 TIME>pno:%d,time:%u",pno,time);                  

                if (0 <= pno && pno <= 1)
                {
                    SrsRtpPacket *pkt_to_send = pkt->copy();
                    pkt_to_send->header.drop_all_extensions();
                    is_arrived_[pkt_to_send->header.get_sequence()] = true;
                    // srs_trace("<DEBUG> enqueue original packet: ssrc=%u, seq=%u, pno=%d, gno=%u", pkt_to_send->header.get_ssrc(), pkt_to_send->header.get_sequence(), pno, gno);
                    if ((err = consumer->enqueue(pkt_to_send)) != srs_success) {
                        return srs_error_wrap(err, "consume message");
                    }
                }

                if (fec_decoder_->should_decode(pkt))
                {
                    SrsRtpPacket *new_pkt = fec_decoder_->xor_decode(pkt);
                    if (new_pkt == nullptr) {
                        return srs_error_new(ERROR_FEC_CODEC, "decoder return null packet");
                    }
                    is_arrived_[new_pkt->header.get_sequence()] = true;
                    // srs_trace("<DEBUG> enqueue recovery packet: ssrc=%u, seq=%u", new_pkt->header.get_ssrc(), new_pkt->header.get_sequence());
                    if ((err = consumer->enqueue(new_pkt)) != srs_success) {
                        return srs_error_wrap(err, "consume message");
                    }
                }
            }
    ...
    //     SrsRtpPacket* pkt_to_send = pkt->copy();
    //     if (consumer->is_sst_upstream_consumer) {
    //         // add SST extension header 
    //         uint16_t osn = pkt_to_send->header.get_sequence();
    //         if (osn % consumer->sdegree_ == consumer->snum_) {
    //             consumer->set_sst_header(pkt_to_send);
    //         }
    //         else {
    //             srs_freep(pkt_to_send);
    //             continue;
    //         }
    //     }
    ...
    pkt->header.set_sequence(oseq);
    pkt->header.set_ssrc(ossrc);
    if (is_sst_downstream_source_) {
        pkt->header.set_pf_info(PF_ID, opno, ogno);
    }
...
void SrsRtcSource::set_stream_desc(SrsRtcSourceDescription* stream_desc)
{
    ...
    // get desc from random init
    if (!get_desc_from_publisher) {
        srs_trace("set stream desc with random init");
        srs_freep(stream_desc_);
    ...
    srs_trace("set stream desc by publisher");
    if (lead_is_set()) {
        sst_additional_descs_.push_back(stream_desc_->copy());
    }
    srs_freep(stream_desc_);
    stream_desc_ = stream_desc->copy();
    stream_desc_->audio_track_desc_->is_active_ = true;
    stream_desc_->video_track_descs_.back()->is_active_ = true;
    sst_lead_audio_ssrc_ = stream_desc_->audio_track_desc_->ssrc_;
    sst_lead_video_ssrc_ = stream_desc_->video_track_descs_[0]->ssrc_;
    get_desc_from_publisher = false;

    // get desc from publisher
    // 以第一个出现的 stream_desc 为主，保留其他 stream_desc
    // srs_trace("set stream desc by publisher");
    // if (!lead_is_set()) {
    //     srs_freep(stream_desc_);
    //     stream_desc_ = stream_desc->copy();
    //     sst_lead_audio_ssrc_ = stream_desc->audio_track_desc_->ssrc_;
    //     sst_lead_video_ssrc_ = stream_desc->video_track_descs_[0]->ssrc_;
    //     get_desc_from_publisher = false;
    // }
    // else {
    //     sst_additional_descs_.push_back(stream_desc->copy());
    //     sst_additional_descs_.back()->audio_track_desc_->is_active_ = true;
    //     sst_additional_descs_.back()->video_track_descs_.back()->is_active_ = true;
    // }

    srs_trace("<BTRACE> set_stream_desc() trace: ");
    trace_sst_descs();
    return;
}
...
srs_error_t SrsRtcRecvTrack::send_rtcp_ack(uint64_t time1, uint64_t time2)
{
    srs_error_t err = srs_success;
    uint32_t ssrc = track_desc_->ssrc_;
    if((err = session_->send_rtcp_ack(ssrc, time1, time2)) != srs_success) {
...
srs_error_t SrsRtcRecvTrack::on_nack(SrsRtpPacket** ppkt)
{
    ...
    // 更新ringbuffer的begin和end
    // 如果seq落在begin和end之间，则nack_first和nack_last不会有变化
    if (!rtp_queue_->update(seq, nack_first, nack_last)) {
        srs_warn("NACK: too old seq %u, range [%u, %u]", seq, rtp_queue_->begin, rtp_queue_->end);
    }
    // seq没有落在 [begin,end) 之间
    if (srs_rtp_seq_distance(nack_first, nack_last) > 0) {
        // if (seq == nack_first) {
        //     srs_trace("<BTRACE> (left out) ssrc=%u, seq=%u, nack range [%u, %u)", track_desc_->ssrc_ , seq, nack_first, nack_last);
        // } else {
        //     // seq == nack_last
        //     srs_trace("<BTRACE> (right out) ssrc=%u, seq=%u, nack range [%u, %u)", track_desc_->ssrc_ , seq, nack_first, nack_last);
        // }

        // 插入序列号为 [nack_first, nack_last) 的包
    ...
    //进行收包统计，送入丢包计算
    ++transmitted_packets;
    int rtt = session_->monitor_->get_rtt(pkt->header.get_ssrc());
    if(rtt > 0 && retrans_seqs.seqs.size() > 0){
        uint64_t now = srs_get_system_time() / 1000;
        for(uint16_t s : retrans_seqs.seqs){
            if(s == seq){
                //对发出nack经过一个rtt以后收到的对应seq包认定为重传包
                if(now > retrans_seqs.timestamp + rtt)
                    ++retrans_packets;
                break;
            }
        }
    }
    session_->monitor_->update_loss_stat(pkt->header.get_ssrc(),rtp_queue_->get_extended_highest_sequence(),transmitted_packets,retrans_packets);
    return err;
}
...
srs_error_t SrsRtcRecvTrack::do_check_send_nacks(uint32_t& timeout_nacks)
{
    ...
    session_->check_send_nacks(nack_receiver_, track_desc_->ssrc_, sent_nacks, timeout_nacks, retrans_seqs);
    return err;
}
...
void SrsRtcSendTrack::update_extension_report_time(SrsNtp& ntp, uint32_t jitter){
    last_xr_ntp = ntp;
    last_xr_sys_time = srs_update_system_time();
    session_->send_rtcp_xr_dlrr(track_desc_->ssrc_, last_xr_sys_time, last_xr_ntp, jitter);
}
...
srs_error_t SrsRtcSendTrack::on_recv_nack(const vector<uint16_t>& lost_seqs)
{
    ...
    for(int i = 0; i < (int)lost_seqs.size(); ++i) {
        ...
        if (pkt == NULL) {
            // srs_trace("<BTRACE> fetch_rtp_packet() miss find pkt");
            continue;
        }
        // srs_trace("<BTRACE> send NACK response: ssrc=%u, seq=%u", pkt->header.get_ssrc(), pkt->header.get_sequence());
