...
void SrsResourceManager::replace_conn_id(const std::string& old_id, const std::string& new_id)
{
    if (conns_id_.count(old_id) != 0) {
        ISrsResource* conn = conns_id_[old_id];
        conns_id_.erase(old_id);
        conns_id_[new_id] = conn;
    }
}
