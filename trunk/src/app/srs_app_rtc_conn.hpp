...
// BUPT section
public:
    bool is_upstream_sst_player_;
    int snum_;
    int sdegree_;
    int sst_id_;
    SrsRtcConsumer* consumer_;
    void get_first_ssrc(uint32_t &video_ssrc, uint32_t &audio_ssrc);
    uint32_t get_first_video_ssrc();
public:
    void trace_player_tracks();
    SrsRtcSource* sst_get_source();
public:
    std::map<uint32_t, SrsNtp> last_recv_ntp_;
    std::map<uint32_t, SrsNtp> last_send_ntp_;
    void update_extension_report_time(uint32_t ssrc, SrsNtp& ntp, uint32_t jitter);
public:
    SrsRtcConnection* session_;
private:
    SrsContextId cid_;
...
private:
    ...
    srs_error_t on_rtcp_ack(SrsRtcpACK* rtcp);
...
// BUPT section
public:
    bool is_downstream_publisher_;
    int snum_;
    int sdegree_;
public:
    srs_error_t disable_sst_downstream_nack();
    srs_error_t refresh_ssrcs(uint32_t audio_ssrc, uint32_t video_ssrc);
private:
...
class SstPublisherSession
{
public:
    SrsRtcConnection* sst_publisher_;
    std::string upstream_ip_;
    int upstream_port_;
    std::string session_id_;
    std::string stream_url_;
public:
    srs_error_t remote_deactivate();
public:
    SstPublisherSession(SrsRtcConnection* sst_publisher);
    virtual ~SstPublisherSession();
public: 
    srs_error_t set_sst_publisher(std::string stream_url, std::string session_id);
    srs_error_t set_upstream_address(std::string upstream_ip, int upstream_port);
};

enum SstConnectionType
{
    NoneSstConnection,
    SstUpstreamPlayerSession,
    SstDownstreamPublisherSession,
    SstDownstreamPlayerSession,
};
...
// MARKG
public:
    // TOFIXG-MONITOR: 挂载monitor功能
    SrsBuptMonitor* monitor_;

    // SstConnectionType used in: on_connection_established(), expire(), deactivate()
    SstConnectionType connection_type_;

    // only SstDownstreamPlayerSession has sst_publishers_
    std::vector<SstPublisherSession*> sst_publishers_;

    std::string stream_url_;
public:
    srs_error_t deactivate_all_sst_upstreams();
    srs_error_t set_skt_addr(std::vector<SrsUdpMuxListener*>& listeners, std::string ip, int port);
    srs_error_t trace_publishers_and_players(std::string base_str);
    srs_error_t sst_update_tracks();
    srs_error_t set_downstream_publisher_info(int snum, int sdegree);
    SrsRtcPlayStream* get_first_play_stream();
    SrsRtcPublishStream* get_first_publish_stream();
    std::string get_username();
    srs_error_t send_rtcp_xr_dlrr(uint32_t ssrc, const uint64_t& last_send_rrtr_systime, const SrsNtp& last_send_rrtr_ntp, uint32_t jitter);
public:
    std::string get_session_peer_id() { return sendonly_skt->peer_id(); };
    srs_error_t refresh_publisher_ssrc_keys(uint32_t new_audio_ssrc, uint32_t new_video_ssrc);
private:
...
public:
    ...
    void check_send_nacks(SrsRtpNackForReceiver* nack, uint32_t ssrc, uint32_t& sent_nacks, uint32_t& timeout_nacks, SrsRtcBuptSeqList& retrans_seqs);
    ...
    srs_error_t send_rtcp_ack(uint32_t ssrc, uint64_t time1, uint64_t time2);
...
private:
    ...
    srs_error_t create_player(SrsRtcUserConfig* ruc, std::map<uint32_t, SrsRtcTrackDescription*> sub_relations);
