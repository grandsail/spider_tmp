...
const int MAX_SUB_STREAM = 10;
const int MAX_SEQ_LIMIT = 65536;
const int SEND_MAP_LEN = 120;
...
// BUPT section
public:
    bool is_sst_upstream_consumer;
    int snum_;
    int sdegree_;
    int sst_id_;

    bool redundant_[MAX_SUB_STREAM] { false };
    bool send_map_[120] { false };

    uint16_t base_seq_;
    bool base_is_set_;
    srs_error_t set_sst_header(SrsRtpPacket *pkt);
    srs_error_t sub_stream_set_seq(SrsRtpPacket *pkt);
    bool sub_stream_should_forward(int bias, uint16_t seq);
    // srs_error_t set_reinjection_rules(int start_reinject, int reinject_index);
    srs_error_t init_send_map(int snum, int sdegree);
    srs_error_t set_send_map(string send_map);
private:
    SrsRtpPacket* ring_buffer[MAX_SEQ_LIMIT] { NULL };
    uint16_t highest_seq_;
    bool highest_is_set_;
    int ring_payload_;
    uint32_t pre_ts_;
public:
    int get_payload() { return ring_payload_; };
private:
    SrsRtcSource* source;
...
// BUPT section
public:
    bool sst_base_is_set;
    uint16_t sst_base_sn;

    bool is_sst_downstream_source_;
    bool get_desc_from_publisher;
    bool called_from_sst_player_;

    uint32_t sst_lead_video_ssrc_;
    uint32_t sst_lead_audio_ssrc_;

    SrsFecEncoder* fec_encoder_;
    SrsFecDecoder* fec_decoder_;

    uint16_t convert_seq_;
    int cdn_fec_pkt_counter_;
    bool header_conversion_is_inited_;
    uint32_t cdn_fec_ssrc_;
    uint16_t convert_gno_;

    uint32_t pre_arrive_seq_;
    bool is_arrived_[MAX_SEQ_LIMIT];
public:
    bool lead_is_set();
    srs_error_t sst_on_source_changed();
    SrsRtcTrackDescription* get_desc_witch_ssrc(uint32_t ssrc);
    SrsRtcTrackDescription* get_additional_desc_with_ssrc(uint32_t ssrc);
    void trace_sst_descs();
    srs_error_t cdn_fec_header_conversion(SrsRtpPacket *pkt);
    bool have_arrived(uint16_t seq);
public:
    ...
    // NOTEG: sst_additional_publishers_ 暂时未被使用，只由主publisher即publish_stream_负责处理PLI
    ISrsRtcPublishStream* publish_stream_;
    std::vector<ISrsRtcPublishStream*> sst_additional_publishers_;

    // Steam description for this steam.
    // NOTEG: stream_desc_ 作为主 desc，被设置时填写sst_lead_video_ssrc_和sst_lead_audio_ssrc_
    // sst_additional_descs_用于查找track并更改ssrc
    SrsRtcSourceDescription* stream_desc_;
    std::vector<SrsRtcSourceDescription*> sst_additional_descs_;
...
public:
    ...
    // NOTEG: 被SrsRtcPlayStream::cycle()调用，在player启动前创建对应的consumer
    ...
    // NOTEG: 在source的consumers中擦除已经删除的consumer
...
public:
    ...
    // NOTEG: 由SrsRtcPlayStream::do_request_keyframe()调用，用于获取source的publisher
    ISrsRtcPublishStream* publish_stream();

    // NOTEG: 设置publisher的回调，放入 publish_stream_
    // SST传输中publish_stream_作为主回调，其他publisher放置在sst_additional_publishers_
    void set_publish_stream(ISrsRtcPublishStream* v);
    
    // Consume the shared RTP packet, user must free it.
    // NOTEG: source接收pkt，填入consumer
    ...
    // NOTEG: player先创建时，只会返回空的stream_desc_
...
public:
    ...
    //加入重传序列号列表，用来判定收到的是否是重传包
    SrsRtcBuptSeqList retrans_seqs;
public:
...
public:
    ...
    bool has_ssrc(uint32_t ssrc);
    void refresh_ssrc(uint32_t ssrc) { track_desc_->set_ssrc(ssrc); }
    uint32_t get_ssrc();
    void update_rtt(int rtt);
    void update_send_report_time(const SrsNtp& ntp, uint32_t rtp_time);
    int64_t cal_avsync_time(uint32_t rtp_time);
    srs_error_t send_rtcp_rr();
    srs_error_t send_rtcp_ack(uint64_t time1, uint64_t time2);
...
public:
    ...
    bool get_track_status();
    std::string get_track_id();
    void update_extension_report_time(SrsNtp& ntp, uint32_t jitter);
public:
    // Note that we can set the pkt to NULL to avoid copy, for example, if the NACK cache the pkt and
    // set to NULL, nack nerver copy it but set the pkt to NULL.
    srs_error_t on_nack(SrsRtpPacket** ppkt);
...
public:
    ...
    uint32_t get_ssrc() { return track_desc_->ssrc_; };
};
