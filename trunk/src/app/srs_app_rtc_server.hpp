...
enum SstPlayerType
{
    NormalPlayer,   //normal no sst browser
    sstDownstreamBrowser,
    sstDownstreamSRS
};
...
public:
    // [1]: sstDownstreamBrowser, [2]: sstDownstreamSRS, [0] NormalPlayer
    SstPlayerType sst_player_;
    // (ip, port) pairs of upstream SRS publishing sub streams
    std::vector<std::pair<std::string, int> > cascade_publishers_;
...
// MARKG server api modify & ruc properties and methods add
public:
    std::string store_sdp_str_;
    std::string remote_sdp_generate(std::string stream_url, std::string sdp_str, int snum, int sdegree);
    SrsSdp remote_play(SrsRtcUserConfig* ruc, std::string request_sdp_str, std::string& sid, int snum);
    srs_error_t refresh_upstream(SrsRtcConnection* session, std::string new_upstream_ip);
// --------------------   MARKG   --------------------
private:
