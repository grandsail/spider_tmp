...
srs_error_t SrsSdp::revert_direction_for_cascade()
{
    for (std::vector<SrsMediaDesc>::iterator iter = media_descs_.begin(); iter != media_descs_.end(); ++iter) {
        for (std::vector<SrsMediaPayloadType>::iterator iter_media = iter->payload_types_.begin(); iter_media != iter->payload_types_.end(); ++iter_media) {
            if (iter->sendonly_) {
                iter->sendonly_ = false;
                iter->recvonly_ = true;
            }
        }
    }
