...
const std::string kSSTExt = "bupt:sub-stream-transmission";
const std::string kPFecExt = "bupt:packet-fec";
const std::string kSendTimeExt = "bupt:send-time";
...
public:
    // used after L2 SRS generate the local SDP, before it send the sdp to L1 SRS
    srs_error_t revert_direction_for_cascade();
// --------------------   MARKG   --------------------
public:
    SrsSdp();
    virtual ~SrsSdp();
